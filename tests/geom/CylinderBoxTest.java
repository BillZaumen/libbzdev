...
public class CylinderBoxTest {
	...
	Path2D box1 = new Path2D.Double();
	box1.moveTo(-100.0, -100.0);
	box1.lineTo(100.0, -100.0);
	box1.lineTo(100.0, 100.0);
	box1.lineTo(-100.0, 100.0);
	box1.closePath();
	...
	BezierGrid grid2 = new
	    BezierGrid(box1, (ind, p, type, bounds) -> {
		    return new Point3D.Double(p.getX(),
					      p.getY(),
					      - 100*ind);  
	    }, 2, false);
	...
	g2d.dispose();
	image.setCoordRotation(Math.toRadians(50.0),
			       Math.toRadians(60.0),
			       0.0);
	...
	m3d.render(image);
	image.write("png", "cylinderbox.png");
