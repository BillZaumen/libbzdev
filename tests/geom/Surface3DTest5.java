...
public class Surface3DTest5 {
	...
	Surface3D.setupU0ForPatch(Path3D.setupCubic(0.0, 0.0, 0.0,
						    0.0, 100.0, 0.0),
				  coords, false);
	Surface3D.setupU1ForPatch(Path3D.setupCubic(100.0, 0.0, 0.0,
						    100.0, 100.0, 0.0),
				  coords, false);
	...
	Surface3D.setupU0ForTriangle(Path3D.setupCubic(100.0, 100.0, 0.0,
						       50.0, 200.0, 0.0),
				     coords, false);
	Surface3D.setupW0ForTriangle(Path3D.setupCubic(0.0, 100.0, 0.0,
						       50.0, 200.0, 0.0),
				     coords, false);
	...
	System.out.println("wellFormed = " + s3d.isWellFormed(System.out));

	Path3D boundary1 = s3d.getBoundary();
	Path3D boundary2 = component.getBoundary();

	if (boundary1 != null) Path3DInfo.printSegments(System.out, boundary1);

	if (boundary1 == null && boundary2 == null) {
	    throw new Exception ("boundary1 and boundary2 are null");
	} else if (boundary1 == null) {
	    throw new Exception ("boundary1 == null");
	} else if (boundary2 == null) {
	    throw new Exception("boundary2 == null");
	}
