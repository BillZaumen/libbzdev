...
public class ConfigPropUtilities {
   ...
   private static Supplier<char[]> defaultPassphraseSupplier = () -> {
    ...
    /**
     * Mode for creating a shared-mime-info file.
     */
    public enum MimeInfoMode {
	/**
	 * The entry will start with directives and elements suitable
	 * for an XML file, followed by a mime-type element.
	 * In other words, the file will start with
	 * <BLOCKQUOTE><PRE><CODE>
	 * &gt;?xml version="1.0" encoding="UTF-8"?&lt;
	 * &gt;mime-info xmlns='http://www.freedesktop.org/standards/shared-mime-info'&lt;
	 * </CODE></PRE></BLOCKQUOTE>
	 * followed by a mime-type element.
	 */
	FIRST_ENTRY,
	/**
	 * The entry will consist of a mime-type element.
	 */
	MIDDLE_ENTRY,
	/**
	 * The entry will consist of a mime-type element, followed by
	 * a closing mime-info element.
	 */
	LAST_ENTRY
    }
    ...
    /**
     * Generate a freedesktop.org shared mime info file for a given
     * media type for a {@link org.bzdev.swing.ConfigPropertyEditor} file.
     * Please see the
     * <A HREF="https://www.freedesktop.org/wiki/Specifications/shared-mime-info-spec/">shared-mime-info specification</A>
     * for a description of the file generated and suggestions for
     * priority values.
     * <P>
     * If a suffix argument starts with a period, that period will be
     * removed: the suffix is interpreted as a file-name extension.
     * @param w the writer to which the file will be written
     ...
     *        isn't one)
     * @param mode the mode ({@link MimeInfoMode#FIRST_ENTRY},
     *        {@link MimeInfoMode#MIDDLE_ENTRY}, or
     *        {@link MimeInfoMode#LAST_ENTRY}); null for a file
     *        containing a single entry
     * @throws IllegalArgumentException for an illegal argument
     * @throws IOException if an IO error occurs
     */
    public static void createMimeInfo(Writer w, int priority,
				      String mediaType,
				      String suffix,
				      MimeInfoMode mode)
    ...
    {
	if (w == null) {
	    throw new IllegalArgumentException(errorMsg("nullWriter"));
	}
	...
	if (mode == null || mode == MimeInfoMode.FIRST_ENTRY) {
	    CopyUtilities.copyResource("org/bzdev/util/SharedMimeInfoStart.xml",
				       w, UTF8);
	}
	String resource = "org/bzdev/util/SharedMimeInfo.xml";
	tp.processSystemResource(resource, "UTF-8", w);
	if (mode == null || mode == MimeInfoMode.LAST_ENTRY) {
