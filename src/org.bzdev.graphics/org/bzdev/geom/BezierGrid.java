 ...
 * </UL>
 * The first argument of these two constructors is a 2D path representing a
...
public class BezierGrid implements Shape3D {
     ...
     * converts each {@link Point2D} control point into a {@link
     * Point3D} with the same X and Y values, and a Z value of 0.0,
     ...
     * not alter the splines in the V direction that were generated by a
     * {@link Point3DMapper}. The mapper will also provide regions. This
     * constructor ensures that the region is the same for all grid points
     * that have the same index along the U direction.
     * <P>
     ...
     * orientation will be correct. Othewise the surface will have to
     * be flipped.
     * <P>
    ...
    /**
     * Constructor based on a 2D path for the V direction and a 3D path
     * for the U direction.
     * <P>
     * The argument <CODE>path</CODE> is a 2D path whose X and Y
     * coordinates are mapped to points along a plane perpendicular to
     * the tangent vector at each knot (i.e., each point that is not
     * an intermediate control point) along the path 'wire'. At each
     * knot, given its tangent T and a previous normal vector
     * N&prime;, the binormal vector B is set to (N&prime; &times; T)
     * and the normal vector is set to (T &times; B). The 2D coordinate
     * (0.0, 0.0) will be mapped to a point on the path <CODE>wire</CODE>.
     * The vector <CODE>inormal</CODE>, which should ideally be
     * perpendicular to the initial tangent vector of the wire, is used as
     * the initial normal vector. If <CODE>inormal</CODE> is null, its
     * value is set the the normal vector of the wire. Then at each knot
     * along the wire, the 2D path <CODE>path</CODE> will be mapped to a
     * points along a plane whose origin is the wire's current knot, whose
     * X axis points along N, and whose Y axis points along B.
     * <P>
     * If <CODE>wire</CODE> is a closed path, the resulting grid will be
     * closed in the U direction and if <CODE>path</CODE> is a closed path,
     * the resulting grid will be closed in the V direction. The grid will
     * be partitioned into regions at the start or end of a wire's straight-line
     * segments or at points at which the wire's normal vector is not
     * continuous. The grid will create splines along the U direction and
     * these will ignore the wire's intermediate control points.
     * @param path the path used as a template for the V direction
     * @param wire the path used to determine positions in the U direction
     * @param inormal the vector that provides an initial normal vector
     *        (can be null if a normal vector can be computed from the 'wire')
     */
    public BezierGrid(Path2D path, Path3D wire, double[] inormal) {
	this(path, getMapper(wire, inormal),
	     Path3DInfo.numberOfDrawableSegments(wire)
	     + ((Path3DInfo.isClosed(wire))? 0: 1), Path3DInfo.isClosed(wire));
    }
     ...
     * incremented by 1 if the path wire is closed.  This mapper
     * treats a two dimensional point (x, y) the same as the three
     * dimenional point (x, y, 0.0), and will map (0.0, 0.0) to a point
     * on the path <CODE>wire</CODE>.
     * <P>
     ...
     * vector given by the argument inormal, except that, when inormal
     * is null, the initial normal vector of the 'wire'is used
     * instead.
     * <P>
     * For points along a 2 dimensional path, an affine transform maps
     * a unit vector in the X direction to N, a unit vector in the Y
     * direction to (N &times; T), and the point (0,0) to the
     * i<sup>th</sup> point along the 3D path <CODE>wire</CODE>,
     ...
     * generated surface. Similarly, if the wire's normal vector is not
     * continuous when crossing segment boundaries, each side will be in
     * a different region.
     * <P>
	...
	while (!pi.isDone()) {
		...
		if (Math.abs(startx - lastx) > 1.e-11
		    || Math.abs(starty - lasty) > 1.e-11
		    || Math.abs(startz - lastz) > 1.e-11) {
